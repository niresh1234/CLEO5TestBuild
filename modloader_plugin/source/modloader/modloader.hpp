...
namespace modloader
{
    ...
    */
    struct plugin : public CLEO_ModLoader_Plugin_t
    {
    };
    ...
    */
    ...
    static_assert(sizeof(plugin) == sizeof(CLEO_ModLoader_Plugin_t), "Invalid plugin inheritance size");
    ...
    */
    ...
    {
        static basic_plugin& GetThis(CLEO_ModLoader_Plugin_t* data)
        {
        ...
        static const char* GetAuthor(CLEO_ModLoader_Plugin_t* data)
        {
            return GetThis(data).GetInfo().author;
        }
        
        static const char* GetVersion(CLEO_ModLoader_Plugin_t* data)
        {
            return GetThis(data).GetInfo().version;
        }
        
        static int OnStartup(CLEO_ModLoader_Plugin_t* data)
        {
            return !GetThis(data).OnStartup();
        }
        
        static int OnShutdown(CLEO_ModLoader_Plugin_t* data)
        {
            return !GetThis(data).OnShutdown();
        }
        
        static int GetBehaviour(CLEO_ModLoader_Plugin_t* data, modloader_file_t* file)
        {
            return GetThis(data).GetBehaviour(GetFile(file));
        }
        
        static int InstallFile(CLEO_ModLoader_Plugin_t* data, const modloader_file_t* file)
        {
            return !GetThis(data).InstallFile(GetFile(file));
        }
        
        static int ReinstallFile(CLEO_ModLoader_Plugin_t* data, const modloader_file_t* file)
        {
            return !GetThis(data).ReinstallFile(GetFile(file));
        }
        
        static int UninstallFile(CLEO_ModLoader_Plugin_t* data, const modloader_file_t* file)
        {
            return !GetThis(data).UninstallFile(GetFile(file));
        }

        static void Update(CLEO_ModLoader_Plugin_t* data)
        {
            return GetThis(data).Update();
        }

        // Attaches the 'interface' using the plugin 'data'
        static void RegisterPluginData(basic_plugin& interfc, CLEO_ModLoader_Plugin_t* data)
        {
    ...
    */
    ...
    #define REGISTER_ML_PLUGIN_PTR(ptr) namespace modloader {\
            ...
            extern "C" __declspec(dllexport) void GetPluginData(CLEO_ModLoader_Plugin_t* data)\
            {\
